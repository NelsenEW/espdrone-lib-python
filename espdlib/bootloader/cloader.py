...
class Cloader:
    ...
    def scan_for_bootloader(self):
        link = espdlib.crtp.get_link_driver('radio://0')
        ts = time.time()
        res = ()
    ...
    def reset_to_bootloader(self, target_id):
        ...
        if got_answer:
            ...
            self.link.close()
            time.sleep(0.2)
            self.link = espdlib.crtp.get_link_driver(
    ...
    def reset_to_bootloader1(self, cpu_id):
        ...
        time.sleep(0.1)
        self.link.close()
        self.link = espdlib.crtp.get_link_driver(self.clink_address)
        # time.sleep(0.1)
    ...
    def open_bootloader_uri(self, uri=None):
        if self.link:
            self.link.close()
        if uri:
            self.link = espdlib.crtp.get_link_driver(uri)
        else:
            self.link = espdlib.crtp.get_link_driver(self.clink_address)
