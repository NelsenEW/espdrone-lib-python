...
class UdpDriver(CRTPDriver):
    ...
    def scan_interface(self, ip):
        addresses = []
        if ip is not None:
            self.ip_scan(ip, addresses)
    ...
    def full_scan():
        ...
        if ip is None:
            return []
        addresses = []
        target = f"192.168.{ip.split('.')[2]}."
        with ThreadPoolExecutor() as executor:
            for id in range(255):
                ip = target + str(id)
                executor.submit(UdpDriver.ip_scan, ip, addresses)
        if addresses:
            return [addresses]
        return []
        ...
        while not self._sp:
            try:
                ...
                if data:
                    ...
                    if (
                        self.link_keep_alive > self.KEEP_ALIVE_MAX_COUNT
                    ) and not self._sp:
                        self._socket.sendto(self._keep_alive_bytearray, self._addr)
            except socket.timeout:
                self._link_error_callback("Connection timeout!")
            except OSError:
